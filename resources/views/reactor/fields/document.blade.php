...
<div class="form-group-column form-group-column--{{ array_get($options, 'fullWidth', false) ? 'full' : 'field' }} ">
    ...
    <div class="form-group__document-container {{ ($document = get_nuclear_documents(request()->old($name, null) ?: $options['value'])) ? '' : ' empty' }}" data-filter="{{ array_get($options, 'filter', 'all') }}">
        @if($document)
